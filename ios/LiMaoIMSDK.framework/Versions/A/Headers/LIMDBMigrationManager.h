//
//  LIMDBMigrationManager.h
//  LIMDBMigrationManager
//
...
@protocol LIMDBMigrating;

/**
 @abstract The `LIMDBMigrationManager` class provides a simple, flexible interface for managing migrations for
 a SQLite database that is accessed via FMDB.
 */
@interface LIMDBMigrationManager : NSObject
...
/**
 ...
 @discussion When `YES` all classes will be enumerated to search for any that conform to the `LIMDBMigrating` protocol.
 */
...
/**
 @abstract Returns all migrations discovered by the receiver. Each object returned conforms to the `LIMDBMigrating` protocol. The
 ...
 and accumulating all classes that conform to the `LIMDBMigrating` protocol. These migrations can then be sorted and applied
 ...
 */
- (id<LIMDBMigrating>)migrationForVersion:(uint64_t)version;
 ...
 */
- (id<LIMDBMigrating>)migrationForName:(NSString *)name;
 ...
 */
- (void)addMigration:(id<LIMDBMigrating>)migration;
...
/**
 ...
 @param migrations An array of objects conforming to `LIMDBMigrating` protocol.
 */
...
/**
 @abstract Creates the `schema_migrations` table used by `LIMDBMigrationManager` to maintain an index of applied migrations.
...
/**
 @abstract The `LIMDBMigrating` protocol is adopted by classes that wish to provide migration of SQLite databases accessed via FMDB. The
 `LIMDBMigrationManager` project ships with a single concrete implementation in the `FMDBFileMigration` class. Non file backed
 migrations can be implemented by conforming to the `LIMDBMigrating` protocol.
 */
@protocol LIMDBMigrating <NSObject>
...
/**
 ...
 @discussion The `LIMDBMigrationManager` manages a transaction while migrations are being applied. Should any call to `migrateDatabase:error` return `NO`,
...
/**
 @abstract The `FMDBFileMigration` class provides a concrete implementation of the `LIMDBMigrating` protocol that models
 ...
 */
@interface LIMDBFileMigration : NSObject <LIMDBMigrating>
...
/**
 @abstract The domain for errors created by `LIMDBMigrationManager`.
 */
extern NSString *const LIMDBMigrationManagerErrorDomain;
 ...
 */
extern NSString *const LIMDBMigrationManagerProgressVersionUserInfoKey;

/**
 @abstract A key for an `id<LIMDBMigrating>` object in the `userInfo` of an `NSProgress` object that identifies
 the migration that was just applied to the database.
 @see `migrateDatabase:error:`
 */
extern NSString *const LIMDBMigrationManagerProgressMigrationUserInfoKey;

/**
 @abstract Enumerates the errors returned by LIMDBMigrationManager
 */
typedef NS_ENUM(NSUInteger, LIMDBMigrationManagerError) {
    /// Indicates that migration was halted due to cancellation
    LIMDBMigrationManagerErrorMigrationCancelled  = 1
};
